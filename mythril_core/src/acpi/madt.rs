...
impl TryFrom<u8> for IcsType {
    type Error = Error;

    fn try_from(value: u8) -> Result<IcsType> {
        match value {
            0x00 => Ok(IcsType::ProcessorLocalApic),
            0x01 => Ok(IcsType::IoApic),
            0x02 => Ok(IcsType::InterruptSourceOverride),
            0x03 => Ok(IcsType::NmiSource),
            0x04 => Ok(IcsType::LocalApicNmi),
            0x05 => Ok(IcsType::LocalApicAddressOverride),
            0x08 => Ok(IcsType::PlatformInterruptSource),
            0x09 => Ok(IcsType::ProcessorLocalX2Apic),
            0x0a => Ok(IcsType::LocalX2ApicNmi),
            _ => {
                Err(Error::InvalidValue(format!("Invalid ICS type: {}", value)))
            }
        }
    }
}
...
pub struct MADT<'a, 'b: 'a> {
    ...
    /// From `ACPI § 5.2.12`:
    ///
    /// > The first byte of each structure declares the type of that structure
...
impl<'a, 'b> MADT<'a, 'b> {
    ...
    pub fn new<'c: 'b>(sdt: &'b SDT<'c>) -> MADT<'a, 'b> {
        let ica =
...
impl<'a, 'b> fmt::Debug for MADT<'a, 'b> {
